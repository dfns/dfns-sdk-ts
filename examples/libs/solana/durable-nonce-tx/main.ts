...
const initDfnsWalletHot = async (walletId: string) => {
  const signer = new AsymmetricKeySigner({
    credId: process.env.DFNS_CRED_ID_HOT!,
    privateKey: process.env.DFNS_PRIVATE_KEY_HOT!,
  })

  const dfnsClient = new DfnsApiClient({
    orgId: process.env.DFNS_ORG_ID_HOT!,
    authToken: process.env.DFNS_AUTH_TOKEN_HOT!,
    baseUrl: process.env.DFNS_API_URL!,
    signer,
  })
...
const main = async () => {
  ...
  if (!nonceAccountAddress) {
    ...
    const hotNonceCreatorWallet = await initDfnsWalletHot(process.env.SOLANA_HOT_NONCE_CREATOR_WALLET_ID!)
    ...
    const message = new TransactionMessage({
      payerKey: hotNonceCreatorWallet.publicKey,
      recentBlockhash: latestBlockhash.blockhash,
      instructions: [
        // account creation
        SystemProgram.createAccount({
          fromPubkey: hotNonceCreatorWallet.publicKey,
    ...
    const signedTx = await hotNonceCreatorWallet.signVersionedTransaction(tx)
  ...
  const amount = Number(process.env.SOLANA_TRANSFER_AMOUNT!)
  const destination = new PublicKey(process.env.SOLANA_TRANSFER_DESTINATION!)

  console.log(`Sending ${amount} lamports to ${destination.toBase58()} with durable nonce`)
